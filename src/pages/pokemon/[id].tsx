...
function pokemonDetail({ pokemon, pokemonSpecies }: Pokemon) {
  return (
    ...
    >
      ...
      <DetailPokemonImage
        pokemonImageId={pokemon.id}
        pokemonName={pokemon.name}
      />
      <DetailDescription
        pokemonDescription={pokemonSpecies.flavor_text_entries[0].flavor_text.replace(
          "",
          " "
        )}
      />
      <DetailWeightAndHeight
      ...
      <DetailTypeSection>
        {
          ...
          pokemon.types.map((pokemon) => {
            return <DetailType type={pokemon.type.name} key={pokemon.id} />;
          })
        }
      </DetailTypeSection>
    </motion.div>
